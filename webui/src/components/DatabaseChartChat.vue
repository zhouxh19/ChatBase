<template>
  <div class="relative rowSC chat-container">
    <el-container style="height: 100%">
      <el-aside width="220px" style="border-right: 1px solid var(--border-color); height: 100%; padding: 20px">
        ...
        <div class="columnBC" style="height: calc(100% - 60px);">
          ...
          <el-button ref="addDatabaseRef" size="large" style="border-radius: 20px!important;" type="primary" plain @click="onAddDatabaseOptionClick">
            Add an database
          </el-button>
        </div>
      </el-aside>
    ...
    >
      <el-tour-step :target="addDatabaseRef?.$el" title="Add Database">
        <div>You can add test databases here, currently only mysql databases are supported.</div>
      </el-tour-step>
    </el-tour>
    ...
    <el-dialog v-model="dialogPromptConfigVisible" title="Database Prompt Config" width="80%" destroy-on-close :close-on-click-modal="false">
      <el-form ref="promptFormRef" size="default" :model="promptConfigForm" label-width="150">
        <el-form-item label="Visulization Prompt" prop="visulization_prompt"
                      :rules="[{required: true, message: 'Please Input Visulization Prompt', trigger: 'blur'}]">
...
import {dbDdlInfoReq, dbExecuteSqlReq, dbGenerateSqlReq} from "@/api/database";
...
const DATABASE_CHART_CHAT_HISTORY_KEY = 'database-chart-chat-history'

const DATABASE_CHART_CHAT_OPTIONS_KEY = 'database-chart-chat-options'

const DATABASE_CHART_CHAT_PROMPT_KEY = 'database-chart-chat-prompt'
...
const addDatabaseRef = ref<ButtonInstance>()
...
onMounted(() => {
  nextTick(() => {
    databaseOptionList.value = JSON.parse(localStorage.getItem(DATABASE_CHART_CHAT_OPTIONS_KEY) || '[]')
    ...
    const promptConfig = JSON.parse(localStorage.getItem(DATABASE_CHART_CHAT_PROMPT_KEY) || '{}')
...
const onEditDatabaseOptionClick = (item) => {
  resetDatabaseForm(JSON.parse(JSON.stringify(item)))
...
const saveDatabaseOptionsToLocalStorage = () => {
  localStorage.setItem(DATABASE_CHART_CHAT_OPTIONS_KEY, JSON.stringify(databaseOptionList.value))
}

const saveDatabasePromptToLocalStorage = () => {
  localStorage.setItem(DATABASE_CHART_CHAT_PROMPT_KEY, JSON.stringify(promptConfigForm))
}
...
const onCreateDatabaseClick = () => {
  ruleFormRef.value.validate((valid) => {
    ...
    if (index >= 0) {
      ...
      databaseOptionList.value.push(JSON.parse(JSON.stringify(editDatabaseForm)))
    }
...
const saveHistoryMessagesToLocal = () => {
  localStorage.setItem(`${DATABASE_CHART_CHAT_HISTORY_KEY}-${databaseSelect.host}-${databaseSelect.database}`, JSON.stringify(messageList.value))
}

const getLocalHistoryMessages = () => {
  const historyMessages = JSON.parse(localStorage.getItem(`${DATABASE_CHART_CHAT_HISTORY_KEY}-${databaseSelect.host}-${databaseSelect.database}`) || '[]')
...
const chat = () => {
  ...
  chatReq(prompt, "", llmModel.value, [], 0, false).then(res => {
