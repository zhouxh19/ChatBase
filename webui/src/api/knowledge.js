...
//获取知识库内的文件列表
export const knowledgeFileDetailsReq = (knowledgeBaseName, page, pageSize) => {
    return axiosReq({
        url: 'knowledge_base/kb_file_details_from_db',
        params: {knowledge_base_name: knowledgeBaseName, page, page_size: pageSize},
        method: 'get'
    })
}

export const knowledgeFileDetailCountReq = (knowledgeBaseName) => {
    return axiosReq({
        url: 'knowledge_base/kb_file_detail_count',
...
//获取知识库内的文件的切片
export const docSplitDetailsReq = (knowledgeBaseName, fileName) => {
    return axiosReq({
        url: 'knowledge_base/doc_split_details',
        data: {knowledge_base_name: knowledgeBaseName, file_name: fileName},
        method: 'post',
        reqLoading: false
    })
}
...
// LLm模型聊天
export const chatReq = (query, conversationId, modelName, history, historyLen, stream = true) => {
    if (!stream) {
        return axiosReq({
            url: 'chat/chat',
            data: {
...
export const knowledgeUploadDocsReq = (knowledgeBaseName, files, chunkSize = 250, chunkOverlap = 50, zhTitleEnhance = false) => {
    ...
    formData.append('background_vectorization', true)
...
//获取正在进行的任务列表
export const uploadActiveTasksReq = (knowledgeBaseName) => {
    return axiosReq({
        url: 'knowledge_base/upload_active_tasks',
        params: {knowledge_base_name: knowledgeBaseName},
        method: 'get',
        reqLoading: false
    })
