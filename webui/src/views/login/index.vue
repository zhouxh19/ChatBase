<template>
  <div class="login-container columnCC">
    <el-form ref="refLoginForm" class="login-form" :model="subForm" :rules="formRules">
      ...
      <el-form-item prop="user" :rules="formRules.isNotNull('usename不能为空')">
        <div class="rowSC">
          <span class="svg-container">
            <svg-icon icon-class="user" />
          </span>
          <el-input v-model="subForm.user" placeholder="用户名" />
          <!--占位-->
          <div class="show-pwd" />
        </div>
      </el-form-item>
...
const loginFunc = () => {
  loginReq(subForm)
      .then(({ data }) => {
        ...
        basicStore.setUserInfo({userInfo: {username: data?.user}})
        router.push('/')
      })
      .catch((err) => {
