...
kb = "kb_for_api_test"
test_files = {
    "wiki/Home.MD": get_file_path("th_document", "wiki/Home.md"),
    "wiki/开发环境部署.MD": get_file_path("th_document", "wiki/开发环境部署.md"),
    "test_files/test.txt": get_file_path("th_document", "test_files/test.txt"),
}
...
def test_update_info(api="/knowledge_base/update_info"):
    url = api_base_url + api
    print("\n更新知识库介绍")
    r = requests.post(url, json={"knowledge_base_name": "th_document", "kb_info": "你好"})
    data = r.json()
    pprint(data)
    assert data["code"] == 200
