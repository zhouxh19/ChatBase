...
def run_controller(log_level: str = "INFO", started_event: mp.Event = None):
    ...
    def release_worker(
            ...
            # worker_address: str = Body(None, description="要释放模型的地址，与名称二选一", th_document=[FSCHAT_CONTROLLER_address()]),
...
def run_celery_worker(started_event: mp.Event = None):
    """
    在独立的进程中启动 Celery Worker。
    """
...
async def start_main_server():
    ...
    celery_started = manager.Event()
    process = Process(
        target=run_celery_worker,
        name=f"Celery Server",
        kwargs=dict(started_event=celery_started),
        daemon=True,
    )
    processes["celery"] = process
    ...
    if process_count() == 0:
        ...
        try:
            ...
            if p:= processes.get("celery"):
                p.start()
                p.name = f"{p.name} ({p.pid})"
                celery_started.wait() # 等待Celery启动完成
