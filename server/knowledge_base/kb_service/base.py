...
from server.db.repository.knowledge_file_repository import (
    ...
    list_docs_from_db, kb_file_details, kb_file_detail_count
)
...
class KBService(ABC):
    ...
    def kb_file_details(self, page=1, page_size=None):
        return kb_file_details(self.kb_name, page, page_size)

    def kb_file_detail_count(self):
        return kb_file_detail_count(self.kb_name)
...
class KBServiceFactory:
    ...
    def get_service(kb_name: str,
        ...
        elif SupportedVSType.OPENGAUSS == vector_store_type:
            from server.knowledge_base.kb_service.opengauss_kb_service import OpengaussKBService
            return OpengaussKBService(kb_name, embed_model=embed_model)
...
def get_kb_details() -> List[Dict]:
    ...
    for kb_detail in kbs_in_db:
        if kb_detail:
            ...
            if kb_detail.get('kb_name') in result:
                result[kb_detail.get('kb_name')].update(kb_detail)
            else:
                kb_detail["in_folder"] = False
                result[kb_detail.get('kb_name')] = kb_detail
...
def get_kb_file_details_from_db(kb_name: str, page=1, page_size=None) -> List[Dict]:
    kb = KBServiceFactory.get_service_by_name(kb_name, DEFAULT_VS_TYPES[0])
    if kb is None:
        return []
    file_details = kb.kb_file_details(page, page_size)
    return file_details

def get_kb_file_detail_count(kb_name: str) -> int:
    kb = KBServiceFactory.get_service_by_name(kb_name, DEFAULT_VS_TYPES[0])
    if kb is None:
        return []
    count = kb.kb_file_detail_count()
    return count
