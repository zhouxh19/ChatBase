...
from server.knowledge_base.kb_service.base import KBServiceFactory, get_kb_file_details, \
    get_kb_file_details_from_db, get_kb_file_detail_count
from server.db.repository.knowledge_file_repository import get_file_detail
from langchain.docstore.document import Document
from typing import List
from server.celery.tasks import celery_upload_docs
...
def api_search_docs(
        ...
        knowledge_base_name: str = Body(..., description="知识库名称", examples=["th_document"]),
...
def search_docs(
    ...
    for vs_type in DEFAULT_VS_TYPES:
        ...
        data.extend(
            [DocumentWithScore(page_content=x[0].page_content, metadata=x[0].dict()['metadata'], score=x[1], vs_type=vs_type) for x in docs])
...
def kb_file_details_from_db(
    ...
    details = get_kb_file_details_from_db(knowledge_base_name, page, page_size)
...
def kb_file_detail_count(
        knowledge_base_name: str,
) -> BaseResponse:
    if not validate_kb_name(knowledge_base_name):
        return BaseResponse(code=403, msg="Don't attack me", data=[])
    knowledge_base_name = urllib.parse.unquote(knowledge_base_name)
    count = get_kb_file_detail_count(knowledge_base_name)
...
#                 knowledge_base_name: str = Form(..., description="知识库名称", examples=["th_document"]),
...
def upload_docs(
        ...
        background_vectorization: bool = Form(False, description="通过后台进行向量化"),
) -> BaseResponse:
    """
    API接口：上传文件，并/或向量化
    """
    ...
    # 对保存的文件进行向量化
    if to_vector_store:
        if background_vectorization:
            celery_upload_docs.delay(file_names, knowledge_base_name, chunk_size, chunk_overlap, zh_title_enhance, not_refresh_vs_cache)
        else:
            for vs_type in DEFAULT_VS_TYPES:
                kb = KBServiceFactory.get_service(knowledge_base_name, vs_type)
                if kb is None:
...
def delete_docs(
        knowledge_base_name: str = Body(..., examples=["th_document"]),
...
def doc_split_details(
        knowledge_base_name: str = Body(..., description="知识库名称", examples=["th_document"]),
        file_name: str = Body(..., description="文件名称，支持多文件", examples=["file_name1"])
):
    """
    下载知识库文档的分篇内容
    """
    ...
    try:
        ...
        if not os.path.exists(kb_file.filepath):
            return BaseResponse(code=403, msg="没有对应的文件")
        text_splitter = None
        splited_docs = kb_file.file2text(text_splitter=text_splitter)
        result = {"file_name": kb_file.filename, "contents": splited_docs}
        return BaseResponse(code=200, msg=f"文档切片内容获取成功", data=result)
    except Exception as e:
...
def download_doc(
        knowledge_base_name: str = Query(..., description="知识库名称", examples=["th_document"]),
