...
from configs import (
    ...
    VECTOR_SEARCH_TOP_K,
    SCORE_THRESHOLD,
    TEMPERATURE, DEFAULT_VS_TYPES, USE_RERANKER, RERANKER_MODEL)
from server.utils import wrap_done, get_ChatOpenAI, get_model_path, embedding_device
...
from server.rerank.rerank import RerankerModel
...
async def knowledge_base_chat(query: str = Body(..., description="用户输入", examples=["你好"]),
    ...
    async def knowledge_base_chat_iterator(
        ...
        # 加入reranker
        if USE_RERANKER:
            reranker_model = RerankerModel(get_model_path(RERANKER_MODEL))
            print("-------------before rerank-----------------")
            print(len(docs))
        ...
        context = ''
        for doc in docs:
            if doc.metadata.get('llm_content') and len(doc.metadata.get('llm_content')) > 0 :
                context += doc.metadata.get('llm_content') + "\n"
            else:
        ...
        for inum, doc in enumerate(docs):
            ...
            if filename in source_documents_map:
                source_documents_map[filename]["contents"].append(json.loads(doc.json()))
            else:
                source_documents_map[filename] = {"filename": filename, "url": url, "contents": [json.loads(doc.json())]}
