...
from server.agent.custom_agent.CourseAgent import initialize_course_agent
...
async def course_agent_chat(query: str = Body(..., description="用户输入", examples=["恼羞成怒"]),
    ...
    async def agent_chat_iterator(
        ...
        print("====model_container====:", model_container)
        agent_executor = initialize_course_agent(
            llm=model,
            tools=tools,
