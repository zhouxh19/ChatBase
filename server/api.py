...
def mount_app_routes(app: FastAPI, run_mode: str = None):
    ...
    # 数据库库相关接口
    mount_database_routes(app)

    # 多源数据相关接口
    mount_all_data_routes(app)
...
def mount_database_routes(app: FastAPI):
    from server.chat.db_data_chat import db_generate_sql, db_ddl_info, db_execute_sql

    app.post("/db/db_execute_sql",
             tags=["DB Data Management"],
             response_model=BaseResponse,
             summary="执行数据库语句")(db_execute_sql)

    app.post("/db/db_ddl_info",
             tags=["DB Data Management"],
             response_model=BaseResponse,
             summary="获取数据库的DDL")(db_ddl_info)

    app.post("/db/db_generate_sql",
             tags=["Chat"],
             summary="与数据库数据对话")(db_generate_sql)


def mount_all_data_routes(app: FastAPI):
    from server.chat.multi_source_data import chat, run_code, upload_file, all_files

    app.post("/multi_source_data/chat",
             tags=["Multi-source Data"],
             response_model=BaseResponse,
             summary="多源数据查询")(chat)

    app.post("/multi_source_data/run_code",
             tags=["Multi-source Data"],
             response_model=BaseResponse,
             summary="多源数据查询-执行代码")(run_code)

    app.post("/multi_source_data/upload_file",
             tags=["Multi-source Data"],
             response_model=BaseResponse,
             summary="多源数据查询-上传文件")(upload_file)

    app.get("/multi_source_data/all_files",
            tags=["Multi-source Data"],
            response_model=BaseResponse,
            summary="多源数据查询-查询所有文件")(all_files)


def mount_user_routes(app: FastAPI):
    from server.user.auth import login
    app.post("/user/login",
             tags=["User"],
             summary="用户登录")(login)
...
def mount_knowledge_routes(app: FastAPI):
    ...
    from server.chat.agent_chat import agent_chat, course_agent_chat
    ...
    from server.knowledge_base.kb_doc_api import (
        list_files,
        kb_file_details,
        kb_file_details_from_db,
        kb_file_detail_count,
        upload_docs,
        delete_docs,
        update_docs,
        download_doc,
    ...
    from server.celery.history import kb_upload_active_tasks
    ...
    app.post("/chat/course_agent_chat",
             tags=["Chat"],
             summary="与course agent对话")(course_agent_chat)
    ...
    app.get("/knowledge_base/kb_file_details_from_db",
            tags=["Knowledge Base Management"],
            response_model=BaseResponse,
            summary="获取保存在数据库内的知识库文件")(kb_file_details_from_db)

    app.get("/knowledge_base/kb_file_detail_count",
            tags=["Knowledge Base Management"],
            response_model=BaseResponse,
            summary="获取保存在数据库内的知识库切片数量")(kb_file_detail_count)
    ...
    app.post("/knowledge_base/doc_split_details",
             tags=["Knowledge Base Management"],
             summary="对应的知识文件的分片详情")(doc_split_details)
    ...
    app.get("/knowledge_base/upload_active_tasks",
            tags=["Knowledge Base Management"],
            summary="获取上传的任务")(kb_upload_active_tasks)
