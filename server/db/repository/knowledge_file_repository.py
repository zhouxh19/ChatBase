...
@with_session
def list_files_from_db(session, kb_name, page=1, page_size=None):
    query = session.query(KnowledgeFileModel).filter_by(kb_name=kb_name)

    # 如果page_size有定义，就进行分页操作
...
@with_session
def kb_file_detail_count(session, kb_name):
    total_docs_count = session.query(func.sum(KnowledgeFileModel.docs_count)) \
        .filter_by(kb_name=kb_name) \
        .scalar()
    return total_docs_count if total_docs_count is not None else 0

@with_session
def kb_file_details(session, kb_name, page=1, page_size=None):
    query = session.query(KnowledgeFileModel).filter_by(kb_name=kb_name)
    ...
    file_dicts = [{
        ...
        'file_ext': f.file_ext,
        'kb_name': f.kb_name,
        'document_loader_name': f.document_loader_name,
        'text_splitter_name': f.text_splitter_name,
        'file_version': f.file_version,
        'file_mtime': f.file_mtime,
        'file_size': f.file_size,
        'custom_docs': f.custom_docs,
